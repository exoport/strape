...
node strape/scripts/guarddog-metadata.mjs      # the 8 registry-metadata rules the tarball sweep cannot reach
node strape/scripts/triage-expectations.mjs --log /tmp/suite.log --hunk "1 (probable)"   # group the unclassified failures
node strape/scripts/sbom.mjs                   # CycloneDX 1.6 of the shipped closure
node strape/scripts/model-catalog.mjs --check  # the vendored catalog is still the reviewed snapshot
