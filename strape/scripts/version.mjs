...
if (args.includes("--check")) {
	...
	// model-catalog is in this list because the vendored model data is the one baseline that can move with no
	// upstream code change at all — its generator reads models.dev live (HUNKS.md hunk 6).
	for (const f of [
		`strape/audit/capability-sweep-${pinTag}.json`,
		`strape/audit/sbom-${pinTag}.json`,
		`strape/audit/model-catalog-${pinTag}.json`,
	]) {
