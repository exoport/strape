...
if (checkPath) {
	...
	const regressions = [];
	// Movement that is real but not a failure: printed so it is never invisible, never exit(1) on its own.
	const contextNotes = [];
	for (const r of results) {
		...
		if (r.scorecard !== null && b.scorecard !== null && r.scorecard <= b.scorecard - 1.0) {
			// Attribute the drop to specific checks before failing. CONTEXT_CHECKS are declared above as
			// "never a flag on their own", but the overall score is a weighted average of everything — so
			// without this, a context check moving alone still failed the gate, contradicting that decision.
			//
			// Maintained is the one that matters: it scores commit/issue activity over a TRAILING 90 DAYS, so
			// it decays for any package that is finished rather than abandoned. Observed 2026-08-11 —
			// jiti@2.7.0 went 5.5 -> 4.5 with Maintained 9 -> 1 and all thirteen other checks byte-identical,
			// 95 days after its release. That is the calendar moving, not the package. A gate that fails on a
			// schedule teaches people to re-baseline without reading, which is how a real drop gets waved past.
			const dropped = Object.entries(r.scorecardChecks)
				.filter(([c, s]) => Number.isFinite(s) && Number.isFinite(b.scorecardChecks?.[c]) && s < b.scorecardChecks[c])
				.map(([c, s]) => ({ check: c, from: b.scorecardChecks[c], to: s }));
			const material = dropped.filter((d) => !CONTEXT_CHECKS.includes(d.check));
			const describe = (list) => list.map((d) => `${d.check} ${d.from}->${d.to}`).join(", ");
			if (material.length || !dropped.length) {
				// !dropped.length means the average moved with no single check regressing — weighting changed,
				// or a check appeared/disappeared. Unexplained movement is exactly what should be looked at.
				regressions.push(
					`${r.name}@${r.version}: Scorecard ${b.scorecard} -> ${r.scorecard}${material.length ? ` (${describe(material)})` : " (no single check regressed — unexplained)"}`,
				);
			} else {
				contextNotes.push(
					`${r.name}@${r.version}: Scorecard ${b.scorecard} -> ${r.scorecard}, explained entirely by context checks (${describe(dropped)})`,
				);
			}
		}
		for (const c of CRITICAL_CHECKS) {
	...
	if (contextNotes.length) {
		console.log(`\n${contextNotes.length} non-material change(s) — reported, not gated:`);
		for (const c of contextNotes) console.log(`  ${c}`);
	}
