...
import { fetchWithRetry } from "./lib/fetch-verified.mjs";
...
const TOOLS = {
	...
	/**
	 * gh is here rather than installed from a distro package ON PURPOSE, and the reason is not supply chain.
	 *
	 * `gh auth login` cannot be disabled — there is no flag or config key for it — and it persists an OAuth
	 * token to ~/.config/gh with far broader scope than the fine-grained token strape/Makefile uses. The way
	 * to not run a command is for it not to be on your PATH. Kept in strape/tools/ and invoked by path, `gh`
	 * is a thing the Makefile can call and not a thing anyone types, so the broad-token path is never taken
	 * by accident. Pinned and hash-verified like the rest, which is the ordinary benefit on top.
	 *
	 * Digest read from api.github.com/repos/cli/cli/releases/tags/v2.97.0 -> assets[].digest on 2026-08-12.
	 */
	gh: {
		version: "v2.97.0",
		repo: "cli/cli",
		binary: "gh",
		assets: {
			"linux-x64": { name: "gh_2.97.0_linux_amd64.tar.gz", sha256: "a2c9b8497e1f85b1ad0dfcb78b5a622e098801b8e461e459e88e1ee12f018112", archive: "tar.gz" },
		},
	},
};
...
for (const name of wanted) {
	...
	let buf;
	try {
		// Transport retries only — the sha256 check below is never retried. See lib/fetch-verified.mjs.
		buf = await fetchWithRetry(url, { log: (m) => console.log(`  ${name}:${m}`) });
	} catch (error) {
		console.error(`  FAIL  ${name}: ${error.message}`);
		if (error.exhausted) console.error(`        Download failure, not a finding — ${name} was never installed.`);
		process.exitCode = 1;
		continue;
	}
