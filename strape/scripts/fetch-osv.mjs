...
let buf;
try {
	// Transport retries only — the integrity check below is never retried. See lib/fetch-verified.mjs.
	buf = await fetchWithRetry(url);
} catch (error) {
	console.error(`Failed to download osv-scanner: ${error.message}`);
	if (error.exhausted) {
		console.error("This is a download failure, not a security finding: the scan never ran.");
		console.error("Re-run the job. If it persists, check GitHub's status before changing the pin.");
	}
	process.exit(1);
}
