...
## 0. If you were handed a release tarball, verify it before you unpack it

Releases from `strape-v0.4.1` onward ship three files: `<name>.tar.zst`, a `.sha256`, and a `.bundle`. Only the
third one proves anything about **origin**. The `.sha256` is published on the same page as the tarball, so
whoever could replace one could replace the other; it detects a corrupted download, not a substituted one.

```sh
cosign verify-blob \
  --bundle strape-<tag>-upstream-<pin>.tar.zst.bundle \
  --certificate-identity "https://github.com/exoport/strape/.github/workflows/strape-release.yml@refs/tags/<tag>" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  strape-<tag>-upstream-<pin>.tar.zst
```

**Both `--certificate-*` flags are mandatory, and this is the whole point.** `cosign verify-blob` with only
`--bundle` will happily accept *any* signature Sigstore ever issued to *anyone* — it confirms the artifact was
signed, not that it was signed **by this repository's release workflow**. Omitting them turns the check into
theatre. Pin the identity to the exact tag you think you are installing.

What a passing verification tells you: this tarball was produced by `strape-release.yml` in `exoport/strape`
at that tag, and the fact is recorded in Rekor's public append-only log. What it does not tell you: that the
tag's contents were reviewed — that is what the signed git tag and `review-attest --verify` cover, in §5.
