...
**Cost:** two expected test failures, both recorded in `strape/audit/expected-test-failures.json`.
`test/system-prompt.test.ts:57` asserts the literal `- When reading pi docs or examples, …`, and
`test/credential-print.test.ts` asserts `Use "pi --help" or "pi auth check …"` — the second arrived with
v0.84.1, which split the auth CLI into `cli/auth-command.ts` and re-hardcoded `pi auth`.

Do **not** credit hunk 3 with the other `pi …` literals in the expected-failure set. Three entries in
`test/package-command-paths.test.ts` assert `pi install` / `pi --help` strings that upstream already wrote as
`${APP_NAME}` (`package-manager-cli.ts:79-89` and `:723`, byte-identical to vendor v0.84.1); those change
because of **hunk 1**, not this hunk. The test is `git show v0.84.1:<file>` — if the literal is unchanged from
vendor, it is hunk 1's rename surfacing, not a string strape edited.
...
(~600K, 1219 models) makes every build offline and turns model/pricing changes into a reviewable git diff
instead of an invisible network fetch from models.dev/OpenRouter at build time.

Refresh deliberately, not automatically: `npm run hydrate:model-data`, then review the diff.

**The catalog is frozen by content** (`strape/audit/model-catalog-<pin>.json`, checked in `strape-build`).
Vendoring closed the *build* hole and left a *provenance* hole: the generator reads
`https://models.dev/api.json` at run time (`packages/ai/scripts/generate-models.ts:1289`, plus NVIDIA NIM at
`:973` and the Vercel AI Gateway at `:1057`), so hydrating during a sync captures models.dev **as of that
day**, not as of the tag being adopted. On 2026-08-09 that is exactly what happened — re-hydration while
adopting v0.84.1 changed the `opencode` model shapes and broke an upstream test's type assumption, arriving
inside a commit labelled "adopt v0.84.1" and indistinguishable from upstream's own work.

`strape/scripts/model-catalog.mjs` pins it: per-file sha256, one digest over the set, and a per-model record
of `baseUrl`, `api`, `cost`, `contextWindow` and `maxTokens`. On drift it names the added/removed/changed
models rather than only reporting that a hash moved — `baseUrl` first and flagged, because a models.dev entry
that repointed a provider would change one string, add and remove no models, and touch no price, while
redirecting the API key and the conversation. It also cross-checks upstream's own `.manifest.json` against the
files on disk, so a catalog edited without regenerating the manifest fails rather than quietly invalidating
`structureHash`. No divergence in `packages/**`: the freeze lives entirely in `strape/`.
