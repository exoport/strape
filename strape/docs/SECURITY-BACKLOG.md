...
## Part 5 — release provenance (workflow only; not a hunk)

Everything above is about the harness. This is about the thing we hand someone else. It is listed separately
because the fix lives in `.github/workflows/strape-release.yml`, which strape owns outright — no hunk, no
invariant, no upstream conflict.

**What exists as of `strape-v0.4.0` (2026-08-11).** The tag is SSH-signed; the signing key is recorded in
`strape/audit/allowed-signers` so `git tag -v` checks against a reviewed list rather than whatever a local
checkout happens to trust, and the same key is registered on GitHub as a signing key so the tag shows as
verified. A GitHub tag ruleset restricts updates and deletions on `strape-v*`, which makes a published release
immutable — the force-push used to add the signature to `v0.4.0` was the last legitimate mutation, and any
future correction has to be a new version.

**What that does and does not prove.** Three artifacts, three different claims, and it is worth being exact
because the gap is easy to mistake for covered:

| Artifact | Proves | Does not prove |
|---|---|---|
| `.sha256` beside the tarball | the download was not corrupted in transit | anything about origin — it is published on the same page, so whoever can replace the tarball can replace the checksum |
| Signed tag | who tagged which commit | that the attached tarball was built from that commit |
| Review attestation | a named human cleared 470 files at a digest | anything about the released binary |

So the released artifact is trust-on-first-download. That is acceptable while the only consumer is the person
who built it. It stops being acceptable the moment someone else installs one.

**The fix: keyless sigstore signing in the release workflow.** `id-token: write` in that workflow's
permissions, a `cosign sign-blob` step after the pack, and `fulcio.sigstore.dev:443` plus `rekor.sigstore.dev:443`
added to its six-endpoint allowlist. `cosign` is already vendored and hash-verified in `strape/tools/` (and
already excluded from the packed tarball), so no new dependency. That yields a verifiable chain from artifact →
workflow run → repository → commit, which is the claim the table above is missing.

**Why keyless rather than a better key.** The current setup uses one SSH key both to authenticate pushes and to
sign tags. It works, and for a single maintainer it is a reasonable trade — but the two roles have very
different blast radii. An authentication-key compromise lets someone push, which the reviewed-deps gate, the
overlay invariants and the review attestation all catch. A signing-key compromise lets someone produce releases
that *look* like ours, and nothing in this stack catches that. The obvious answer is a dedicated signing key on
hardware; the better answer is to have no long-lived signing key at all. Keyless OIDC binds the artifact to the
workflow that built it, which removes key custody as a problem rather than relocating it.

Do this before the first release anyone else consumes, and not in the same change as a release — a workflow with
`contents: write` is not one to debug while cutting a tag.

### DONE 2026-08-11 — implemented, not yet exercised on a real tag

`strape-release.yml` now carries `id-token: write`, fetches the pinned cosign via `fetch-tool.mjs` (sha256
`4629c757…`, re-verified in the job), runs `cosign sign-blob --bundle`, and then **verifies the bundle it just
produced in the same job** — pinning `--certificate-identity` to this workflow's own ref and
`--certificate-oidc-issuer` to `https://token.actions.githubusercontent.com`. A signature that is attached but
unverifiable is worse than none, because it reads as provenance; verifying in-job is what stops that. The
`.bundle` is attached to the release alongside the tarball and its `.sha256`.

Three decisions worth keeping:

1. **The signing step is not tag-gated.** Everything else that matters here was unobservable until a real tag,
   which is how the `api.github.com` / `uploads.github.com` entries ended up predicted rather than audited. A
   `workflow_dispatch` rehearsal now exercises the OIDC exchange, Fulcio, Rekor and the TUF fetch, so the five
   new allowlist entries can be confirmed before a tag depends on them. Cost: a Rekor entry for a rehearsal
   artifact, which is ordinary in a public append-only log.
2. **`oauth2.sigstore.dev` is deliberately NOT allowlisted.** Verified locally: with no ambient OIDC token
   cosign prints *"Non-interactive mode detected, using device flow"* and waits on a browser code. In CI that
   fallback could only hang or sign under the wrong identity, so blocking the host makes it fail closed.
3. **`*.actions.githubusercontent.com` is a wildcard because the OIDC host is regional and rotates** — the
   same class of moving target as `productionresultssa2.blob.core.windows.net`.

**Rehearsed 2026-08-11.** A `workflow_dispatch` run against `main` reached
`Signing artifact… Wrote bundle… Verified OK` under `egress-policy: block`, and `Attach to release` correctly
**skipped** — so the tag guard holds and the five new endpoints are observed rather than predicted. Notably
cosign went straight from *"Generating ephemeral keys"* to *"Signing artifact"* with no device-flow prompt,
which is the positive evidence that the ambient Actions OIDC credential was used and the deliberately-omitted
`oauth2.sigstore.dev` was never needed.

**Remaining gap:** the rehearsal cannot reach `Attach to release`, so `api.github.com` / `uploads.github.com`
stay the only unconfirmed entries in this workflow — unchanged from before, and unavoidable while that step is
tag-gated.
