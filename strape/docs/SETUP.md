...
```sh
gh api repos/exoport/strape/dependabot/alerts --paginate \
  --jq '.[] | select(.state=="open") | [.number, .dependency.manifest_path, .security_advisory.severity] | @tsv'

# For each alert whose manifest_path is under packages/coding-agent/examples/:
gh api -X PATCH repos/exoport/strape/dependabot/alerts/<N> \
  -f state=dismissed -f dismissed_reason=not_used \
  -f dismissed_comment="Example extension; hunk 2 removes it from the workspaces so it is never built or shipped. Shipped closure (npm-shrinkwrap.json) scans clean."
```
